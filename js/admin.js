...
(function () {
  ...
  const RENDER = {
    ...
    admins(P, A) {
      ...
      const rows = (data.admins || []).map((a) => `<tr><td><b>${esc(a.email)}</b><small>${esc(a.name || "")}</small></td><td><span class="pill ${a.role === "master" ? "ok" : ""}">${esc(a.role)}</span></td><td>${(a.groupIds || []).map((id) => { const g = (data.groups || []).find((x) => x.id === id); return esc(g ? g.name : id); }).join(", ") || (a.role === "master" ? "All groups" : "—")}</td><td>${esc(String(a.addedAt).slice(0, 10))}</td><td class="actions">${a.role === "master" ? "" : `<button class="btn btn-text" data-act="editAdmin" data-id="${a.email}">Edit</button><button class="btn btn-text btn-danger" data-act="delAdmin" data-id="${a.email}">Remove</button>`}</td></tr>`);
